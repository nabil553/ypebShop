...
<!--筛选-->
    <div class="filter">
    	...
    	<c:forEach items="${categoryList}" var="category">
         <a href="frontShopping_IndexOther_queryByTwoMenu?categoryID=${category.id}" target="index"><span class="name">${category.name}</span></a>
         </c:forEach>
...
<div class="floor">
	<c:forEach items="${goodsList}" var="goods">
		<div class="foot">
			...
			<div class="price">
					<c:if test="${user.style==2}" ><h1 class="vip_price">商城会员价:${goods.discountPrice}</h1><h2>普通会员价:${goods.price}</h2></c:if>
