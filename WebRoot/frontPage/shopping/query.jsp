...
<link rel="stylesheet" type="text/css"
	href="frontCss/shopping/query.css">
	...
	<div class="query_all">
		...
		<div class="floor">
			...
			<c:forEach items="${goodsList}" var="goods">
				<div class="foot">
					<div class="img_box">
						<a href="frontShopping_Goods_queryGoods?id=${goods.id }"
							target="index"> <img class="foot_img"
							src="upload/image/goods/${goods.imageUrl1 }">
						</a>
					</div>
					...
					<div class="price">
						<c:if test="${user.style==2}">
							<h1 class="vip_price">
								商城会员价:${goods.discountPrice}
								<h2>普通会员价${goods.price}</h2>
							</h1>
						</c:if>
						<c:if test="${user.style==1 || user==null}">
							<h2>普通会员价${goods.price}</h2>
						</c:if>
					</div>
					<input type="hidden" value="${goods.id }"> <img
						class="foot_shopping_cart"
						src="image/index/foot_shopping_cart.png" shopID="${goods.id }">
				</div>
			</c:forEach>
			<div class="item">
				<ul class="pageUl">
					<c:if test="${(page.currentPage-1)>0}">
						<li><a class="firstpage"
							href="frontShopping_IndexOther_queryLike?pageNum=${page.currentPage-1}">&laquo;</a></li>
					</c:if>
					...
					<c:if test="${page.currentPage<page.totalPage}">
						<li><a class="endpage"
							href="frontShopping_IndexOther_queryLike?pageNum=${page.currentPage+1}">&raquo;</a></li>
					</c:if>
				</ul>
			</div>
			<div class="clearfix"></div>
