		...
		<div class="searchBar">
			...
			<ul>
				<li><label>订单状态：</label><select class="combox"
					name="order.state">
						<option value="">订单状态(${order.state })</option>
	...
	<div class="panelBar">
		<ul class="toolBar">
			...
			<a class="delete"
				...
				target="ajaxTodo" title="确定已经发货吗?"><span>发货</span></a>
			</li>
			<a class="edit" href="backShopping_GoodsOrder_preview?uid={sid_user}"
				target="dialog" width="900" height="600"><span>查看商品清单</span></a>
			</li>
		</ul>
		...
		<tbody>
			...
			<c:forEach items="${orderAddress}" var="list">
				<tr target="sid_user" rel="${list.order.id}">
					...
					<td><c:if test="${list.order.style==false }">普通会员订单</c:if> <c:if
							test="${list.order.style==true }">商城会员订单</c:if></td>
